...
"""
Knowledge agent for the Intelligent Knowledge Aggregation Platform.
...
# Import specific knowledge graph modules
from agents.knowledge.graph_builder import GraphBuilder
from agents.knowledge.relation_extractor import RelationExtractor
...
class KnowledgeAgent:
    """Knowledge agent for building and maintaining the knowledge graph."""
    ...
    def __init__(self, agent_id: str, config_path: Optional[str] = None):
        ...
        self.knowledge_validator = KnowledgeValidator(self.config)
        
        # Track current task
        self.current_task_id = None
        self.is_running = False
        
        logger.info(f"Knowledge agent {agent_id} initialized")
    
    async def start(self) -> None:
        """Start the knowledge agent."""
        if self.is_running:
            logger.warning(f"Knowledge agent {self.agent_id} is already running")
            return
            
        logger.info(f"Starting knowledge agent {self.agent_id}")
    ...
    async def stop(self) -> None:
        ...
        logger.info(f"Stopping knowledge agent {self.agent_id}")
    ...
    async def _process_task(self, task: Dict[str, Any]) -> None:
        ...
        try:
            # Determine which knowledge component to use based on task type
            result = None
            
            if task_type == "build_graph":
                result = await self.graph_builder.build(task_data)
            elif task_type == "extract_relations":
                result = await self.relation_extractor.extract(task_data)
            elif task_type == "validate_knowledge":
                result = await self.knowledge_validator.validate(task_data)
            else:
...
async def main():
    """Main entry point for running the knowledge agent."""
    import argparse
    
    parser = argparse.ArgumentParser(description="Run a knowledge agent")
    parser.add_argument("--agent-id", type=str, help="Agent ID")
    parser.add_argument("--config", type=str, help="Path to configuration file")
    
    args = parser.parse_args()
    
    agent_id = args.agent_id or f"knowledge-{os.getpid()}"
    
    agent = KnowledgeAgent(agent_id, args.config)
